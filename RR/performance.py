...
def main():
    ...
    if args.dataset == 'wpc':
        ...
        file_list = [[f"wpc_{name}_patch512" for name in group]for group in common_names]
    elif args.dataset == 'sjtu' or args.dataset == 'icip':
        file_list = [[x] for x in sorted(os.listdir(test_dir))]
    elif args.dataset == 'ls-pcqa':
        score = pd.read_csv(os.path.join(test_dir, 'score.csv'))
        mos = pd.read_csv(args.mos_dir)
        mos['name'] = mos['name'].str.replace('.ply', '')
        score_filtered = score[score['filename'].isin(mos['name'])]
        score_filtered['filename_no_ply'] = score_filtered['filename'].str.replace('.ply', '')
        score_filtered = score_filtered.set_index('filename_no_ply')
        score_reordered = score_filtered.loc[mos['name']].reset_index(drop=True)
        output_path = os.path.join(test_dir, 'score1.csv')
        score_reordered.to_csv(output_path, index=False)
    ...
    for files in tqdm(file_list):
        ...
        score_all = pd.read_csv(os.path.join(test_dir,'score1.csv'))['quality'].to_numpy()
        mos_list = []
        ...
        with open(args.mos_dir, mode='r', newline='') as file:
            ...
            next(reader)
            for row in reader:
                mos_list.append(float(row[1]))
        mos_all = np.array(mos_list)
        print('!!!debug',mos_all.shape,score_all.shape)
        for i in tqdm(range(len(dex)-1)):
            score = score_all[dex[i]:dex[i+1]]
            mos = mos_all[dex[i]:dex[i+1]]
            score = fit(score, mos)
            plcc, srcc = map(lambda func: func(score, mos)[0], [stats.pearsonr, stats.spearmanr])
